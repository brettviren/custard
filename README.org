...
*custard* also provides *boost streams* for *zip* with the heavy lifting by
[[https://github.com/richgel999/miniz/][miniz]].

- [[file:custard_boost_miniz.hpp][~custard_boost_miniz.hpp~]]

This needs the included ~miniz.hpp~ file which is created from the
miniz "amalgamated" source.

#+begin_example
cp miniz/amalgamation/miniz.h miniz.hpp
tail +2 miniz/amalgamation/miniz.c >> miniz.hpp
#+end_example 

The ~tail~ chops off the ~#include "miniz.h"~.
